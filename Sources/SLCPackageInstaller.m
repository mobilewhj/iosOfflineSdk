...
#import <sys/stat.h>
#import <SLCOfflineSDK/SLCOfflineSDK-Swift.h>

NSErrorDomain const SLCOfflineErrorDomain = @"com.offline.demo";
NSString * const SLCOfflineErrorStageKey = @"stage";
NSString * const SLCOfflineHTTPStatusKey = @"httpStatus";

NSError *SLCError(SLCOfflineErrorCode code, NSString *stage, NSString *message, NSError *cause) {
    NSMutableDictionary *info = [@{NSLocalizedDescriptionKey:message, SLCOfflineErrorStageKey:stage} mutableCopy];
    if (cause) info[NSUnderlyingErrorKey] = cause;
    return [NSError errorWithDomain:SLCOfflineErrorDomain code:code userInfo:info];
}
...
@implementation SLCPackageRecord
...
@interface SLCPackageInstaller ()
@property NSURL *requestedRoot;
@property dispatch_queue_t queue;
@end

@implementation SLCPackageInstaller
- (instancetype)initWithRootDirectory:(NSURL *)root {
    if ((self = [super init])) {
        _requestedRoot = [root copy];
        _queue = dispatch_queue_create("com.offline.demo.install", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
- (NSURL *)prepareRoot:(NSError **)error {
    ...
    if (!input.isFileURL || input.path.length <= 1) {
        *error = SLCError(SLCOfflineInvalidInput, @"prepare", @"安装根目录无效", nil); return nil;
    }
    ...
    if (lstat(root.fileSystemRepresentation, &info) == 0 && !S_ISDIR(info.st_mode)) {
        *error = SLCError(SLCOfflineFileIO, @"prepare", @"安装根目录不是普通目录", nil); return nil;
    }
    NSError *io;
    if (![[NSFileManager defaultManager] createDirectoryAtURL:root withIntermediateDirectories:YES attributes:nil error:&io] ||
        ...
        *error = SLCError(SLCOfflineFileIO, @"prepare", @"安装目录创建失败", io); return nil;
    }
    return root;
}
- (NSProgress *)installRecord:(SLCPackageRecord *)record fromURL:(NSURL *)url progress:(SLCInstallProgress)progress completion:(SLCInstallCompletion)completion {
    return [self install:record source:url remote:YES progress:progress completion:completion];
}
- (NSProgress *)installRecord:(SLCPackageRecord *)record localArchive:(NSURL *)archive progress:(SLCInstallProgress)progress completion:(SLCInstallCompletion)completion {
    return [self install:record source:archive remote:NO progress:progress completion:completion];
}
- (NSProgress *)install:(SLCPackageRecord *)record source:(NSURL *)source remote:(BOOL)remote
               progress:(SLCInstallProgress)progress completion:(SLCInstallCompletion)completion {
    NSProgress *cancellation = [NSProgress progressWithTotalUnitCount:1];
    SLCInstallProgress report = ^(NSString *stage, int64_t done, int64_t total) {
...
- (NSURL *)performInstall:(SLCPackageRecord *)record source:(NSURL *)source remote:(BOOL)remote
             cancellation:(NSProgress *)cancellation report:(SLCInstallProgress)report error:(NSError **)error {
    NSString *stage = @"prepare";
    if (![record isKindOfClass:SLCPackageRecord.class] ||
        ![[SLCPackageRecord alloc] initWithVersion:record.version sha256:record.sha256] || !source ||
        ...
        *error = SLCError(SLCOfflineInvalidInput, stage, @"资源包参数无效", nil); return nil;
    }
    if (cancellation.cancelled) { *error = SLCError(SLCOfflineCancelled, stage, @"资源安装已取消", nil); return nil; }
    report(stage, 0, -1);
    ...
    if (lstat(published.fileSystemRepresentation, &info) == 0) {
        *error = SLCError(SLCOfflineVersionExists, @"publish", @"版本目录已存在，不允许覆盖", nil); return nil;
    }
    ...
    do {
        ...
        if (remote) {
            if (![[SLCDownload new] downloadURL:source destination:archive cancellation:cancellation progress:report error:&failure]) break;
        ...
        if (!digest) break;
        if (![digest isEqualToString:record.sha256]) { failure = SLCError(SLCOfflineDigestMismatch, stage, @"资源包 SHA-256 校验失败", nil); break; }
        stage = @"extract"; report(stage, 0, -1);
        if (![SLCArchiveReader extractArchive:archive toDirectory:content cancellation:cancellation progress:^(int64_t done, int64_t total) {
            report(@"extract", done, total);
        } error:&failure]) {
            if (![failure.domain isEqualToString:SLCOfflineErrorDomain])
                failure = SLCError([failure.domain isEqualToString:NSCocoaErrorDomain] ? SLCOfflineFileIO : SLCOfflineInvalidArchive,
                                   stage, @"资源包解压失败", failure);
            break;
        }
        NSURL *entry = [content URLByAppendingPathComponent:@"index.html"];
        if (!SLCRegularNonemptyFile(entry)) {
            content = [content URLByAppendingPathComponent:@"dist" isDirectory:YES];
            if (!SLCRegularNonemptyFile([content URLByAppendingPathComponent:@"index.html"])) {
                failure = SLCError(SLCOfflineInvalidArchive, stage, @"资源包缺少非空 index.html", nil); break;
            }
        }
        stage = @"publish";
    ...
    NSError *cleanup;
    if (![fm removeItemAtURL:staging error:&cleanup] && !failure) failure = SLCError(SLCOfflineFileIO, @"cleanup", @"安装临时文件清理失败", cleanup);
    // A cancelled/failed publication is an unbound residual, never a successful install.
    if (cancellation.cancelled) failure = SLCError(SLCOfflineCancelled, stage, @"资源安装已取消", failure);
    if (failure || !publishedOK) {
        if (publishedOK) [fm removeItemAtURL:published error:nil];
        *error = [failure.domain isEqualToString:SLCOfflineErrorDomain] ? failure :
                 SLCError(SLCOfflineFileIO, stage, @"资源包安装失败", failure);
        return nil;
    }
    return published;
}
- (BOOL)copyArchive:(NSURL *)source to:(NSURL *)destination cancellation:(NSProgress *)cancel error:(NSError **)error {
    ...
    while ((count = [input read:buffer maxLength:sizeof(buffer)]) > 0) {
        ...
        if (cancel.cancelled || total > 64LL * 1024 * 1024) {
            *error = SLCError(cancel.cancelled ? SLCOfflineCancelled : SLCOfflineTooLarge, @"download", @"资源包取消或超过 64 MiB", nil); ok = NO; break;
        }
        NSInteger offset = 0;
        while (offset < count) {
    ...
    if (count < 0 || !ok) {
        if (!*error) *error = SLCError(SLCOfflineFileIO, @"download", @"资源文件复制失败", input.streamError ?: output.streamError);
        ok = NO;
    }
...
- (NSString *)digest:(NSURL *)url cancellation:(NSProgress *)cancel error:(NSError **)error {
    ...
    if (cancel.cancelled || count < 0) { *error = SLCError(cancel.cancelled ? SLCOfflineCancelled : SLCOfflineFileIO, @"verify", @"摘要读取失败或取消", readError); return nil; }
...
- (void)clearOldVersionsKeeping:(NSNumber *)activeVersion completion:(void (^)(NSError *))completion {
    dispatch_async(self.queue, ^{
        ...
        if (root && activeVersion && (activeVersion.integerValue < 10000 || lstat(active.fileSystemRepresentation, &info) != 0 || !S_ISDIR(info.st_mode) ||
            !SLCRegularNonemptyFile([active URLByAppendingPathComponent:@"index.html"]))) {
            error = SLCError(SLCOfflineInvalidInput, @"cleanup", @"当前版本不可用，拒绝清理", nil);
        }
        if (root && !error) {
            NSFileManager *fm = [NSFileManager defaultManager];
...
- (void)discardUnboundVersion:(NSInteger)version completion:(void (^)(NSError *))completion {
    dispatch_async(self.queue, ^{
        NSError *error;
        if (version < 10000) error = SLCError(SLCOfflineInvalidInput, @"cleanup", @"版本号无效", nil);
