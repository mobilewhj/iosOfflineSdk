...
@interface SLCOfflineResource ()
@property (nonatomic, copy) NSURL *fileURL;
@property (nonatomic, copy) NSString *MIMEType;
@property (nonatomic, copy) NSString *textEncodingName;
@end
@implementation SLCOfflineResource
@end
@interface SLCOfflineResourceResolver ()
@property NSURL *directory;
@property NSURLComponents *base;
@end
@implementation SLCOfflineResourceResolver
static BOOL SLCSafePath(NSString *path) {
...
static NSInteger SLCPort(NSURLComponents *url) { return url.port ? url.port.integerValue : ([url.scheme.lowercaseString isEqualToString:@"https"] ? 443 : 80); }
- (instancetype)initWithDirectory:(NSURL *)directory baseURL:(NSURL *)baseURL {
    ...
    if (!directory.isFileURL || lstat(directory.fileSystemRepresentation, &info) != 0 || !S_ISDIR(info.st_mode) ||
        ...
        base.user || base.password || base.query || base.fragment || ![base.path hasSuffix:@"/"] || !SLCSafePath(base.path)) return nil;
    if ((self = [super init])) { _directory = directory.URLByResolvingSymlinksInPath; _base = base; }
    return self;
}
- (SLCOfflineResource *)resourceForRequest:(NSURLRequest *)request {
    ...
    if (!url.scheme.length || !url.host.length || [url.scheme caseInsensitiveCompare:self.base.scheme] != NSOrderedSame ||
        [url.host caseInsensitiveCompare:self.base.host] != NSOrderedSame || SLCPort(url) != SLCPort(self.base) ||
        url.user || url.password || !SLCSafePath(url.path) || ![url.path hasPrefix:self.base.path]) return nil;
    ...
    if (![file.URLByResolvingSymlinksInPath.path hasPrefix:rootPrefix] || !SLCRegularNonemptyFile(file)) return nil;
    ...
    if (!type) return nil;
    SLCOfflineResource *resource = [SLCOfflineResource new]; resource.fileURL = file; resource.MIMEType = type;
