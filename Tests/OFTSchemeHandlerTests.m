...
@interface OFTTestSchemeTask : NSObject <WKURLSchemeTask>
...
@end
@implementation OFTTestSchemeTask
...
@end
@interface OFTSchemeHandlerTests : XCTestCase
@property NSURL *directory;
@property OFTOfflineSchemeHandler *handler;
@end
@implementation OFTSchemeHandlerTests
- (void)setUp {
    ...
    self.handler=[[OFTOfflineSchemeHandler alloc] initWithDirectory:self.directory baseURL:[NSURL URLWithString:@"https://demo.example/app/"] scheme:@"offline-test"];
    XCTAssertNotNil(self.handler);
}
- (void)tearDown { [[NSFileManager defaultManager] removeItemAtURL:self.directory error:nil]; }
- (OFTTestSchemeTask *)task:(NSString *)address {
    OFTTestSchemeTask *task=[OFTTestSchemeTask new]; task.request=[NSURLRequest requestWithURL:[NSURL URLWithString:address]]; return task;
}
- (void)testReservedAndInvalidSchemesAreRejected {
    for (NSString *scheme in @[@"http",@"https",@"file",@"1bad",@"bad scheme",@""]) {
        XCTAssertNil([[OFTOfflineSchemeHandler alloc] initWithDirectory:self.directory baseURL:[NSURL URLWithString:@"https://demo.example/app/"] scheme:scheme]);
    }
}
...
- (void)testResponseUsesLocalBytesAndDoesNotGrantCORS {
    OFTTestSchemeTask *task=[self task:@"offline-test://demo.example/app/index.html"];
...
- (void)testMissingForeignPostAndRangeRequestsFailWithoutNetworkFallback {
    for (NSString *kind in @[@"missing",@"foreign",@"post",@"range"]) {
        ...
        OFTTestSchemeTask *task=[self task:address]; NSMutableURLRequest *request=task.request.mutableCopy;
...
- (void)testStopFromResponseCallbackPreventsDataAndFinish {
    ...
    __weak OFTTestSchemeTask *weakTask=task;
